...
- 首页不再显示原先顶部的介绍块。
- “知识助手”区域并列展示制造一厂和制造四厂两个入口。
- 制造一厂说明文字为“知识来源为云盘内相关文档”。
- 制造四厂说明文字为“知识来源为制造四厂 RAGFlow 知识库”。
- 首页显示问答助手、翻译助手等入口和使用次数。
- 首页只显示今日活跃访问；后端仍保留日、月、年访问统计字段。
- 所有用户均可通过顶部按钮提交问题反馈；只有管理员登录后，首页才显示问题意见列表和回复功能。
- 反馈列表接口同样要求管理员会话，普通用户不能通过直接访问 API 查看反馈内容。
...
```text
GET    /api/health
GET    /api/assistants/ragflow/open
GET    /api/me
POST   /api/login
POST   /api/logout
POST   /api/visit
...
其中 `POST /api/feedback` 对普通用户开放；`GET /api/feedback` 和回复接口仅管理员可用。
...
- 桌面端和移动端布局无明显横向溢出。
- 浏览器自动化确认页面无 iframe、四厂入口使用 `_blank`、两个问答统计独立显示且桌面端无横向溢出。
...
- 需求：部署服务器地址由 `172.28.200.119` 更换为 `172.28.200.66`。
- 实现：更新项目现状和访问地址记录。
- 数据或配置影响：无。Nginx 监听所有本机地址，Node API 监听 `0.0.0.0:4178`，运行代码没有写死服务器 IP。
...
### 2026-09-16 - 增加制造四厂 RAGFlow 知识问答助手

- 需求：把内网 RAGFlow 中创建的助手加入现有平台，名称为“制造四厂知识问答助手”，与制造一厂知识问答助手放在一起。
- 实现：首页知识助手区域增加四厂入口；问答工作台增加一厂、四厂切换；四厂页面通过官方共享聊天 iframe 嵌入，并提供加载状态、重新加载和新窗口兜底；新增独立四厂使用事件，首页问答次数显示两厂合计。
- 数据或配置影响：不修改数据库结构；`.env` 新增 `RAGFLOW_CHAT_NAME` 和 `RAGFLOW_CHAT_URL`，完整地址包含认证参数且不提交 GitHub。
- 验证：`npm run lint`、`npm run build`、`git diff --check` 通过；Edge 自动化验证两个首页入口、双助手切换、iframe 地址读取、统计合计，以及桌面端和 390px 移动端无横向溢出。当前电脑不在内网，RAGFlow 真实会话需在服务器网络中验证。
- Git 提交：`d203c0b`。
- 部署包：已生成 `E:\Qwen-cc\aizhushou-nginx-windows-20260916.zip`，大小 `102.69 MB`，SHA-256 为 `DF5F4343875A8F959E41669A8E64EAC0B582CD7848514C4A3AD704CD1ADE5F39`；Nginx 配置检查、发布目录依赖检查、首页静态资源、`/api/health` 和 `/api/assistant-config` 反向代理联调通过。

### 2026-09-16 - 四厂助手改为新窗口打开并拆分统计

- 需求：解决 RAGFlow 在 iframe 中回答明显慢于新窗口的问题；点击四厂助手时直接打开新窗口，并把一厂、四厂问答使用次数分开统计。
- 实现：删除四厂 iframe 页面、加载状态和双助手切换栏；首页四厂卡片改为原生 `_blank` 外链；后端新增固定目标跳转接口，先记录四厂使用事件再返回 `302`；首页增加两张独立问答统计卡。
- 数据或配置影响：不修改数据库结构，继续使用已有 `qa_click` 和 `ragflow_click` 事件；当前只需要 `RAGFLOW_CHAT_URL`。
- 验证：`npm run lint`、`npm run build`、`git diff --check` 通过；接口验证 `302` 目标、禁止缓存、来源页策略和单次计数；浏览器自动化确认无 iframe、外链属性正确、两个统计项独立且桌面端无横向溢出。
- Git 提交：`67ca54d`。
- 部署包：已生成 `E:\Qwen-cc\aizhushou-nginx-windows-20260916-v2.zip`，大小 `102.84 MB`，SHA-256 为 `3F560724B6C700B02825B656787130B1CAFF6193FF9095C0FB9F3A111CFFA142`；Nginx 配置、发布依赖、首页静态资源、跳转接口和分离统计联调通过。

### 2026-09-16 - 移除月度和年度活跃卡片

- 需求：从首页统计指标中去掉“本月 / 本年活跃”。
- 实现：删除对应统计卡片，保留今日活跃访问及其他助手使用次数。
- 数据或配置影响：无。后端仍保留月、年访问统计字段，数据库不变。
- 验证：`npm run lint`、`npm run build`、`git diff --check` 通过，源码和构建结果中均不再包含该卡片文字。
- Git 提交：`a75c8a0`。
- 部署包：本次未重新生成；`20260916-v2` 发布包不包含这项界面调整。

### 2026-09-16 - 问题意见仅管理员可见

- 需求：隐藏首页“问题意见”整个区域，只有管理员登录后才能显示和查看。
- 实现：未登录时不渲染问题意见区域、不请求反馈列表；管理员登录后加载列表，退出后立即清空浏览器中的反馈数据；后端读取反馈接口增加管理员权限校验。普通用户仍可提交反馈。
- 数据或配置影响：无，不修改数据库结构和既有反馈数据。
- 验证：`npm run lint`、`npm run build`、`git diff --check` 通过；未登录首页无问题意见区域；未登录读取接口返回 `401`，管理员会话读取返回 `200`。
- Git 提交：`f91d954`。
- 部署包：本次未重新生成；`20260916-v2` 发布包不包含这项权限调整。

### 2026-09-16 - 生成 v3 Windows 发布包

- 目的：将移除月度/年度活跃卡片和问题意见管理员权限两项最新修改部署到内网服务器。
- 包含功能提交：`a75c8a0`、`f91d954`。
...
### 2026-09-17 - 修复 HTTP 200 内的业务 NotFound

- 现象：v2 日志显示 `/api/v1/chat/completions -> HTTP 200`，页面仍收到 `<NotFound '404: Not Found'>`。
- 原因：该 RAGFlow 版本把路由或业务 404 包装在 HTTP 200 的 SSE 数据中，v2 只依据 HTTP 状态码判断是否回退。
- 实现：将旧版 `chats_openai` 调整为首选；通过双路流读取首个 SSE 事件，发现业务 NotFound 后不中断浏览器连接，自动尝试下一兼容接口。
- 数据或配置影响：数据库和 `.env` 不变。
